...
export const gerarAnalogia: VerbalGenerator = (seed, difficulty) => {
  ...
  return {
    ...
    answerId,
    explanation:
      `A relação é **${RELATION_LABEL[base.relation]}**: ${base.a} → ${base.b}. ` +
      `A alternativa correta (${correta.a} → ${correta.b}) é a única que repete essa ` +
      `mesma relação. O método é sempre o mesmo: monte uma frase que ligue as duas ` +
      `palavras do enunciado ("${base.a} é ${RELATION_LABEL[base.relation]} ${base.b}") ` +
      `e teste essa frase em cada alternativa. Se a frase não encaixa, elimine.`,
    satisfiesRule,
  }
}
...
function gerarVocabOposicao(
  ...
  return {
    ...
    explanation:
      subtipo === 'antonimo'
        ? `"${entry.word}" significa algo próximo de "${entry.synonyms[0]}". ` +
          `O oposto é "${correta}". Repare que "${armadilhas[0]}" também está entre as ` +
          `alternativas: é sinônimo, não antônimo — a pegadinha mais comum da prova é ` +
          `marcar o sinônimo por ler o enunciado rápido demais. Leia OPPOSITE antes de olhar as opções.`
        : `"${entry.word}" significa algo próximo de "${correta}". ` +
          `Cuidado com "${armadilhas[0]}", que é o antônimo e está ali justamente para ` +
          `quem lê o enunciado no automático. Confirme se a pergunta pede SIMILAR ou OPPOSITE ` +
          `antes de escolher.`,
    satisfiesRule,
  }
}
...
export const gerarCompletarFrase: VerbalGenerator = (seed, difficulty) => {
  ...
  return {
    ...
    answerId,
    explanation:
      `A resposta é "${frame.answer}". ${frame.rationale} ` +
      `Regra geral: antes de olhar as alternativas, decida que TIPO de palavra a frase ` +
      `pede (positiva ou negativa, forte ou fraca). Conectivos como "although", "despite" ` +
      `e os dois-pontos são o que denuncia isso.`,
    satisfiesRule,
  }
}
